...
@AndroidEntryPoint
class EditCartFragment : Fragment(), OnMapReadyCallback {

    lateinit var binding : FragmentEditCartBinding
...
//                cartViewModel.saveIdCart(idseller)
                findNavController().navigate(R.id.action_editCartFragment_to_cartFragment)
                Toast.makeText(requireContext(), "Berhasil update cart", Toast.LENGTH_SHORT)
                    .show()
            } else {
                Toast.makeText(requireContext(), "Gagal Update cart", Toast.LENGTH_SHORT)
                    .show()
            }
        }
    }
